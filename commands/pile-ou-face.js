...
async function playGame(interaction, playerChoice, opponentChoice = null, player = null, opponent = null) {
    ...
    collector.on('collect', async i => {
        ...
        await interaction.commandName;
        
        // Créer un nouvel embed pour inviter à utiliser la commande à nouveau
        const replayEmbed = new EmbedBuilder()
            .setTitle(`${emoji} Nouvelle partie`)
            .setDescription(`Pour jouer à nouveau, utilisez la commande \`/pile-ou-face\``)
            .setColor(color)
            .setFooter({ text: 'Pile ou Face' })
            .setTimestamp();
